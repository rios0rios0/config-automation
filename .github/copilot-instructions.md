...
`config-automation` runs scheduled, cross-repo maintenance against every repository of every owner listed in `HARDEN_OWNER` — a comma-separated list, currently [`rios0rios0`](https://github.com/rios0rios0), [`medhub-tech`](https://github.com/medhub-tech), and [`prefy`](https://github.com/prefy). Repositories are identified by their `owner/name` slug (`entities.Repository.QualifiedName()`), since bare names collide across owners:
...
| Variable                         | Purpose                                                                 |
|----------------------------------|-------------------------------------------------------------------------|
| `HARDEN_OWNER`                   | Comma-separated GitHub owners/orgs to audit, in order (default: `rios0rios0`; workflows use `rios0rios0,medhub-tech,prefy`). |
