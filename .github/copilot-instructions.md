...
Workflow secrets, one fine-grained PAT per owner (a fine-grained token is bound to a single resource owner, and each token's lifetime must be 366 days or less):
`COMPLIANCE_AUDIT_TOKEN` / `MEDHUB_TECH_COMPLIANCE_AUDIT_TOKEN` / `PREFY_COMPLIANCE_AUDIT_TOKEN` (daily audit),
`CLAUDE_MD_REFRESH_TOKEN` / `MEDHUB_TECH_REFRESH_TOKEN` / `PREFY_REFRESH_TOKEN` (refresh discover + PRs, and release-reconcile tag pushes),
`CLAUDE_CODE_OAUTH_TOKEN` (refresh Claude Code CLI, shared).
