...
2. **Weekly config and docs refresh** — `.github/workflows/config-and-docs-refresh.yaml` enumerates non-fork non-archived repos via `go run ./cmd/harden-repos --list-json`, chunks them into batches of `batch_size` (default `10`) so the matrix has O(repos / batch_size) legs. Each leg installs `@anthropic-ai/claude-code` via `npm` plus a pinned, checksum-verified `chlog` release, loads `scripts/refresh_config_and_docs_prompt.md` from the self-checkout, and loops through its batch internally — cloning each target repo and invoking `claude -p ... --max-turns "${MAX_TURNS}" --allowedTools '...' </dev/null` (the `</dev/null` is load-bearing: without it `claude` inherits the loop's stdin from `jq` and drains the batch after the first repo). `claude` output is tee'd to `${WORK_DIR}/.claude.log` so the loop can detect the org-wide `monthly usage limit` message and short-circuit the rest of the batch (skipped repos surface on a `quota_skipped` summary line; the quota-hitting repo is added to `failed` as `claude-quota:` so the leg still goes red). A Claude safeguard refusal (`safeguards flagged` / `cyber-use-case`) is deterministic for that one repo, so it surfaces on a `safeguard_skipped` summary line plus a `::warning::` annotation and does not fail the leg; every other failure kind still does. Drift detection uses `git add -N` + `git diff -w --quiet` on the in-scope files (today: `CLAUDE.md`, `.github/copilot-instructions.md`, and the tailored Copilot code-review skill at `.github/skills/code-review/SKILL.md`); the changelog target is staged with them but excluded from the gate. The in-scope set is duplicated in three places that must be changed together — `drift_paths`, the step's `--allowedTools` grant, and the file list in `scripts/refresh_config_and_docs_prompt.md`. The changelog target is picked per repo and enforced by the grant, not by the prompt: `changelog_mode=chlog` when the clone has `.chlog.yaml`, `.chlog.yml`, or `.changes/unreleased/` — the same predicate the `pipelines` basic-checks gate and AutoBump's `DetectChlog` use (grant `Bash(chlog new:*)` — the command rather than write access to the directory — and the prompt tells Claude to run it), `changelog` when it only has `CHANGELOG.md` (grant `Edit(/CHANGELOG.md)`), `none` otherwise. Hand-editing a generated `CHANGELOG.md` is what the 2026-08-31 run did to 46 chlog repositories; withholding the grant is what makes it impossible now, and a post-refresh guard reverts any `CHANGELOG.md`/existing-fragment change in chlog mode and fails the repo with `fragment:` on a malformed new fragment or one whose nanosecond filename and `time` disagree (a hand-typed fragment; `chlog new` writes both from clock reads microseconds apart), or `changelog-guard:` when a restore itself fails (flagged, not bare — a bare command in an `if` body would abort the whole leg under `errexit`). Every file grant is an `Edit(...)` rule — the CLI matches file permissions on `Edit` alone (it covers `Write` too); a `Write(...)` rule is inert and only warns. A second post-refresh guard reads the ADDED lines of the drifted in-scope files (`git diff -U0` after `git add -N`, so a brand-new file counts in full) and fails the repo with `secret-shape:` when one carries a credential shape — the hyphenated Slack bot-token prefix, a PEM private-key banner, a completed GitHub or AWS prefix, a JWT-shaped string. It fails closed by construction — the diff is captured before it is searched (an unreadable diff is a `secret-shape:` rejection, not a pass) and the match stage is `grep -E … >/dev/null`, never `grep -Eq`, whose early exit would hand the `if` the upstream grep's SIGPIPE status under `pipefail` and silently skip the guard. The shared `sast:gitleaks` stage matches shapes, not values, and its full-history scan on `main` cannot be cleared by a later edit, only by a `.gitleaksignore` fingerprint this job is granted no tool to write; scanning added lines only keeps the fleet's older, still-hyphenated skills from failing runs that did not touch them. `scripts/refresh_config_and_docs_prompt.md` states the same rule and fixes the one approved wording for the skill's secret-hygiene bullet — prompt and guard move together. Before the `claude` call, each leg reads `git log -1 --first-parent -- . :(exclude)<every path this job writes>` and skips a repo whose last code change predates `stale_after_days` (default 8, one day past the weekly cadence), reporting it on the `unchanged` summary line: the excludes are derived from `IN_SCOPE` so the job's own refresh/bump/changelog commits never count as activity, `--first-parent` reports a merge's date rather than a long-lived branch's authoring date, and a repo carrying none of the in-scope files is never skipped; `stale_after_days: 0` forces a full-fleet pass, which a newly added refresh target needs. Branch name `chore/config-and-docs-refresh` is force-pushed to keep one open PR per repo. Each leg prints a `==== Batch summary ====` footer (`processed / unchanged / no_drift / prs_created / prs_updated / quota_skipped / safeguard_skipped / failed`) and exits non-zero only when a repo landed in `failed`. `workflow_dispatch` exposes `repo`, `batch_size`, `max_parallel`, `max_turns`, and `stale_after_days` inputs (defaults: `10`, `2`, `50`, `8`). The workflow is named for the broader scope so future refresh targets (diagrams, more config files) can be added without renaming.
...
- **`api/issues/bulk_change` reports a refusal in its 200 body, not in the status line.** It answers with a `total`/`success`/`ignored`/`failures` tally, and an issue the caller may not transition (the token's user lacks `Administer Issues` there) lands in `ignored`. `SonarCloudSonarProjectsRepository.AcceptIssues` therefore decodes the tally, returns how many actually moved, and errors on any ignored or failed one; `ApplySonarPolicyCommand` logs that number rather than `len(keys)`. Treating a 2xx as success would log "26 issues accepted", exit 0, and leave all 26 open — the sibling write paths (`api/settings/set`, `api/hotspots/change_status`) only avoid that hole because they answer 403.
