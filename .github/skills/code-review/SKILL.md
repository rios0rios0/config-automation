...
- **Ruleset compliance is a three-part check**: name match, the `non_fast_forward` rule, and `refs/heads/main` in the ref-name include list. A name-only match is not compliant. `BypassActors` must retain `RepositoryAdminActorType` / `RepositoryAdminActorID` or the owner loses force-push.
