---
name: code-review
description: "Review pull requests and diffs in config-automation — the scheduled cross-repo hardening and refresh automation — against the rios0rios0/guide standards, with extra weight on the compliance-policy invariants, the per-owner PAT matrix, and SHA-pinned actions. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `config-automation`

`config-automation` audits and hardens every repository of `rios0rios0`, `medhub-life`, and `prefy` on a schedule, refreshes their AI-assistant docs, and reconciles missing releases. A policy change here propagates to every repository on the next run, so the blast radius of a wrong edit is the whole estate.
...
## What matters most in `config-automation`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **`AuditResult.ComputeIssues()` is the single source of truth for compliance.** Every carve-out lives there — forks skip Dependabot and secret scanning; private repos skip secret scanning, branch protection, and rulesets; `DesiredWikiAllowlist()` repos keep `has_wiki=true`; `AllowAutoMerge=true` is skipped on private repos because GitHub Free ignores the `PATCH`. A carve-out added anywhere else is a Critical finding.
- **Policy constants live in `internal/domain/entities/compliance_policy.go`.** `DesiredRepoSettings()` and `DesiredWikiAllowlist()` are functions returning fresh values so call sites cannot mutate the policy — turning one back into a package variable is a Critical finding.
- **`Repository.FindAllByOwner` has three branches** — authenticated self (`/user/repos`, which retains private visibility), `OwnerKind=User`, and `OwnerKind=Organization`. All three must stay in sync; a change to one only is a defect.
- **Ruleset compliance is a four-part check** (`Ruleset.IsCompliant`): name match, the `non_fast_forward` rule, `allowed_merge_methods` on the `pull_request` rule equal to `DesiredAllowedMergeMethods()` (`["merge"]`), and `refs/heads/main` in the ref-name include list. A name-only match is not compliant, and `non_fast_forward` (blocks *force pushes*) is not `allowed_merge_methods` (restricts how a pull request lands) — conflating them leaves the fast-forward merge path open. `BypassActors` must retain `RepositoryAdminActorType` / `RepositoryAdminActorID` or the owner loses force-push.
- **The merge shape spans two layers.** `DesiredRepoSettings()` keeps `AllowRebaseMerge` on (the only toggle that surfaces *Update with rebase*) alongside `AllowMergeCommit` on, `AllowSquashMerge` off, and `AllowUpdateBranch` on; the ruleset's `allowed_merge_methods` narrows `main` to merge commits. A ruleset can only narrow what the repo allows, so weakening either half — dropping `AllowRebaseMerge`, or widening `DesiredAllowedMergeMethods()` beyond `["merge"]` — re-opens the fast-forward or kills the button, and is a defect.
...
- **`SecuritySettingsRepository.FindByRepositoryName` returns `DependabotAlerts *bool`** where `nil` means "unknown / API failure" and pointer-to-false means "disabled". Collapsing the two turns an outage into a false compliance report.
...
- `//go:build unit`, `_test` package suffix, `t.Parallel()` on every top-level test function.
- In-memory doubles under `test/domain/doubles/repositories/` are preferred over any mock library; builders live in `test/domain/builders/`.
- A compliance carve-out without a policy test that pins it is incomplete.
...
- The verbosity of the phase commands — the phase split is deliberate.
- Workflow YAML using single quotes and `.yaml` — that is the standard, not an error.
