...
- added a secret-shape guard to `config-and-docs-refresh.yaml`: after drift detection it reads the added lines of the drifted in-scope files and skips the repository with `secret-shape:` when one carries a credential shape — the hyphenated Slack bot-token prefix, a PEM private-key banner, a completed GitHub or AWS prefix, or a JWT-shaped string. The prompt rule alone depends on the model complying, and the cost of a lapse is a finding that only a `.gitleaksignore` fingerprint can clear, which this job is granted no tool to write. It reads added lines only, so the fleet's older skills — which still carry the old wording — do not fail runs that never touched them. The guard is wired to fail closed: the diff is captured before it is searched, so one that cannot be read rejects the repository instead of passing it silently, and the match never uses `grep -q`, whose exit on the first hit would hand the check the upstream SIGPIPE status under `pipefail` and skip it without a word
...
- stopped the weekly config-and-docs refresh from writing a gitleaks finding into every repository it touches. The code-review skill it generates warns against credential-shaped fixtures, and until now nothing told it how to phrase that warning, so it spelled the vendor prefixes out — including the hyphenated Slack bot-token prefix, which the GitLab-customised rule set the shared `sast:gitleaks` stage runs matches on its own with no body check. 39 repositories carry that sentence today and each one fails the stage the moment it adopts a language pipeline, because the scan on `main` walks the whole history reachable from `HEAD`. `scripts/refresh_config_and_docs_prompt.md` now forbids credential-shaped literals in every file the refresh writes, fixes one approved wording for that bullet whose prefixes stop short of what the rules match, and treats an existing shape-quoting bullet as drift to be replaced; this repository's own `.github/skills/code-review/SKILL.md` was aligned to that wording
...
- stopped the weekly configuration and documentation refresh from re-examining a repository whose code has not moved. 47 of the fleet's 76 repositories opened a pull request on 2026-08-31 — 62%, one week after the same job had refreshed them — because the prompt's "only if meaningfully drifted" bar is a judgement the model re-makes from scratch every run: `rios0rios0/srt` yielded two entries on 2026-08-31 and none on 2026-09-01 against an unchanged `main`. The refresh now reads, after the clone and before the ~3-minute `claude` call, the date of the last commit that touched anything outside its own output (`CLAUDE.md`, `.github/copilot-instructions.md`, `.github/skills/code-review/SKILL.md`, `.changes/`, `CHANGELOG.md`) and skips the repository when that is older than `stale_after_days`, default 8. The signal is read with `--first-parent`, without which pathspec history simplification reports a long-lived branch's authoring date instead of its merge date and skips that repository on every run thereafter; a repository carrying none of the in-scope files is never skipped, since "quiet" only means "up to date" once the job has produced something there; and `stale_after_days` reaches the step raw and is defaulted in bash, because GitHub's `||` treats the numeric `0` as falsy and would have made the documented `0` escape hatch inoperative — one day past the weekly cadence. Excluding the refresh's own output is the whole design: the default branch's HEAD date is self-defeating, because merging a refresh pull request moves `main` and makes every repository the job touched look busy the following week, which is the loop the gate exists to break. Skipped repositories are reported on an `unchanged` summary line, a single-repo `workflow_dispatch` run is never skipped, and `stale_after_days: 0` restores the old behaviour

### Fixed

- stopped the weekly configuration and documentation refresh from hand-editing a generated `CHANGELOG.md`. Every target repository uses [chlog](https://github.com/luizjhonata/chlog), where `CHANGELOG.md` is compiled from the fragments under `.changes/unreleased/` — so an entry written straight into it is discarded by the next `chlog batch`, and it puts back on that one file exactly the rebase conflicts the fragments exist to prevent. The 2026-08-31 run did that to 46 repositories at once, because the prompt told Claude to edit the changelog and the tool grant let it. The refresh now decides the convention per repository — `chlog`, `changelog`, or `none` — using the same predicate as the `pipelines` basic-checks gate and AutoBump's `DetectChlog` (`.chlog.yaml`, `.chlog.yml`, or a `.changes/unreleased/` directory), appends the matching half of the prompt's new `Recording the change` section to that repository's prompt, and grants only the tool that convention needs: `Edit(/.changes/unreleased/**)`, `Edit(/CHANGELOG.md)`, or nothing. Withholding the grant is what makes the wrong target impossible rather than merely discouraged; a post-refresh guard additionally reverts any `CHANGELOG.md` or existing-fragment change in a chlog repository — recording a `changelog-guard:` failure and skipping the repository when a restore itself fails, rather than letting it open a PR carrying the very edit the guard exists to prevent — and rejects a new fragment missing a `kind:`, `body:`, or `time:` line with a `fragment:` failure. Every grant is now spelled `Edit(...)` as well, because the CLI matches file permissions on `Edit` rules alone — the four `Write(...)` entries the allowlist carried had never granted anything
