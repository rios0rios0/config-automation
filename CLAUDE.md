...
- **The merge flags are a shape, not three independent toggles.** `AllowMergeCommit`+`AllowRebaseMerge` on and `AllowSquashMerge` off is how a semi-linear history is spelled on GitHub: rebase the PR onto its base (*Update with rebase*, which GitHub only surfaces while rebase merging is enabled), then land it with a merge commit. Turning squash back on, or turning rebase merging off to hide the fast-forwarding *Rebase and merge* button, each break it in a different direction — change all three together or not at all.
